...
- `SUPABASE_STORAGE_BUCKET` — имя bucket для хранения скриншотов шагов на сервере (API upload и серверные операции).
- `NEXT_PUBLIC_SUPABASE_STORAGE_BUCKET` — имя bucket для клиентской части (viewer/client upload); обычно равно `SUPABASE_STORAGE_BUCKET`.
